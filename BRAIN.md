...
- 2026-05-10: Multi-label BRAIN corrections (DirtySnatcha/WHOiSEE/Dark Matter/Kotrax) — see memory `reference_managed_artists_multi_label.md`. Dashboard 5-vs-8 fix: `is_managed=true` filter applied to scope resolver + Xai bot + chat_bridge.py; default scope now `mgmt` (commit `6e9127d` tenx10, `abaf66d` umbrella). Andrew Lehr sweep across 9 stragglers + BUILD_EVOLUTION correction (commits `5fa370c` / `e875830` / `44d66f1`).
- 2026-05-10: Spotify daily pipeline for the 4 missing roster artists (commit `c329bcb`). New shared runner at `MANAGEMENT-TENx10/scripts/spotify-daily/multi_pull.py` pulls DSR + WHOiSEE + Dark Matter + Kotrax in one pass; HVRCRFT keeps its standalone script til post-event. `artists.spotify_artist_id` populated for all 5 via Supabase MCP. `spotify_daily_snapshots` now has 7 rows (HVRCRFT x3 + 4 fresh) — `/dashboard/artists` drawer now works for ALL 5 managed cards on Vercel. Scheduled-task registration documented in README; deferred until post-event.
- 2026-05-10: Spotify cadence upgrade to option C — direct Supabase upsert + Oracle VM systemd timer (commits `54d9305`, `<HVRCRFT-add>`). `multi_pull.py` writes straight to `spotify_daily_snapshots` after each artist (no separate backfill on critical path). VM service `spotify-daily.service` + timer `spotify-daily.timer` fire at 9am America/New_York daily, `Persistent=true` (catches up if VM was down). HVRCRFT added to VM REGISTRY so all 5 are laptop-independent. Local Windows `HVRCRFT-Spotify-Daily` task still runs for the NotebookLM push (retire post-event once folded in). Drawer cadence is now "0–24 hours fresh, no laptop required."
