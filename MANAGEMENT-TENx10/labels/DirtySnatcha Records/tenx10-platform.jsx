...
// ─────────────────────────────────────────────────────────────────────
// DUAL AI ROUTER
// Claude  = builder / artifact layer  (claude.ai, dev)
// Gemini  = production consumer layer (deployed app)
// Automatically detects environment and routes accordingly.
// Both layers use the same system prompts and message format.
// ─────────────────────────────────────────────────────────────────────
const AI_PROVIDER = (() => {
  try {
    return window.location.hostname.includes("claude.ai") ? "claude" : "gemini";
  } catch { return "claude"; }
})();

const GEMINI_API_KEY = typeof import_meta_env !== "undefined"
  ? import_meta_env.VITE_GEMINI_API_KEY || ""
  : "";
...
function ChatTab({ entity }) {
  ...
  const sendGreeting = async () => {
    setLoading(true);
    try {
      const text = await callAI(entity.systemPrompt, [
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs" style={{color:"rgba(245,158,11,0.75)"}}>
                  Thomas Nalian · 4 Artists · DirtySnatcha Records
                </div>
              </div>
            </div>
