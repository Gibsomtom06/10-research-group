# DirtySnatcha — Brain (project brief / context file / north star doc)

Living context for DirtySnatcha as a managed artist.

---

## Who

**DirtySnatcha = Leigh Bray** (legal name; sometimes spelled "Lee" informally). All booking contracts signed **"Leigh Bray aka DirtySnatcha"** as Licensor.

**Ownership of the artist project:** Leigh Bray owns **100% of DirtySnatcha** (the artist).

**Management:** **Thomas Nalian** manages DirtySnatcha. Thomas receives **10% of total gross income** for management. Thomas Nalian and Leigh Bray are two different people — never conflate them. The artist signs contracts; the manager runs ops.

Genre: Dubstep, Riddim, Bass Music, Trap.
Label home: DirtySnatcha Records (DSR) — a co-owned label (Leigh Bray 70% / Thomas Nalian 30%). See `MANAGEMENT-TENx10/labels/DirtySnatcha Records/BRAIN.md` for label ownership detail.
Distributor: VMG (Virgin Music Group).
PRO: BMI. IPI: 01017500116.
Publisher: LAB10 Publishing (BMI).

Artist contact: contact@dirtysnatcha.com · 586-208-6886
Manager contact: thomas@dirtysnatcha.com (Thomas Nalian)

---

## Current Metrics (verify before using)

- ~136 tracks in catalog
- ~14M lifetime streams
- Spotify Popularity Score: 28
- ~8–9K monthly listeners
- ~4,500 Spotify followers · 11K Instagram

---

## Booking

- Primary agent: Andrew Bass (AB Touring) — andrew@abtouring.com
- Legacy agent: Colton Anderson (PRYSM) — colton@prysmtalentagency.com · 734-904-0224 — being transitioned out
- Floor guarantee: $1,500
- Commission: agent-routed 10/10/80, direct 20/80
- Active tour: TMTYL 2026 — 17 shows, ~$38,600 guaranteed
- **Electric Forest 2026** — late June 2026 (confirmed playing)
- **Lost Lands 2026** — late September 2026 (confirmed playing)

## Merch — Pre-Sell Campaign

- MHP producing custom DSR merch drop (design done, art file pending)
- Storefront: dirtysnatcharecords.com/merch
- **Goal: 100 units pre-sold BEFORE Electric Forest**
- Context: proving demand > 100 to DirtySnatcha before the festival
- Merch designed around Electric Forest + Lost Lands drops
- RJ Jackson (CMO agent) owns campaign execution

---

## Publishing Gaps

- LAB10 Publishing: 0 of 82 tracks registered at MLC — critical, nothing submitted yet
- SoundExchange: not registered as performer
- CMRRA: not filed
- International mechanicals: not registered with any society yet
